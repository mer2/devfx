	...
	public abstract class Converting : IConverting
	{
		...
		Type IConverting.ToType(bool throwError) {
			...
			try {
				...
				if(throwError) {
					throw new ExceptionBase("转换成Type时错误", e);
				}
			}
			return type;
		}
		...
		public static T StringToEnum<T>(string enumString) {
			var type = typeof(T);
			if (!type.IsEnum) {
				throw new ArgumentException("类型必须是枚举", "enumString");
			}
		...
		internal class ConvertingInternal : Converting
		{
			...
			private readonly string convertingValue;

			protected override string ConvertingValue {
				get { return this.convertingValue; }
			}
		}
	}
}
...
namespace HTB.DevFx.Config
{
	...
	public abstract partial class ConfigSetting : IConfigSetting
	{
		...
		/// <summary>
		/// 配置节来源的设定，形如：{file:'htb.devfx.app.config', node:'/configuration/htb.devfx/customSetting', provider:'HTB.DevFx.Config.XmlConfig, HTB.DevFx'}
		/// </summary>
		...
		public virtual ConfigSetting GetChildSetting(params string[] settingName) {
			...
			if(settingName != null && settingName.Length > 0) {
				for(var i = 0; setting != null && i < settingName.Length; i++) {
					var name = settingName[i];
					switch (name) {
						case ".":
						case null:
							break;
